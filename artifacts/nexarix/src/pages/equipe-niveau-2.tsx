...
export default function EquipeNiveau2() {
  const { user } = useAuth();
          ...
          <div className="relative z-10 p-5 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-blue-200 text-[10px] font-bold uppercase tracking-widest">Mon Équipe</p>
                  <p className="text-white font-black text-lg leading-tight">Niveau 2</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-white font-black text-3xl leading-none">{count}</p>
                <p className="text-blue-200 text-[11px] font-semibold mt-0.5">membre{count > 1 ? "s" : ""}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-2xl border border-white/10 p-3 text-center" style={{ background: "rgba(255,255,255,0.08)" }}>
                ...
                <p className="text-blue-200 text-[10px] font-bold uppercase tracking-wider mb-1">Commission</p>
                <p className="text-white font-black text-xl leading-none">{CONFIG.commission.toLocaleString()}</p>
                <p className="text-blue-300 text-[10px] font-semibold mt-1">{currency} / filleul actif</p>
              </div>
              <div className="rounded-2xl border border-white/10 p-3 text-center" style={{ background: "rgba(255,255,255,0.08)" }}>
                <Users className="h-3.5 w-3.5 mx-auto mb-1.5 text-yellow-300" />
                <p className="text-blue-200 text-[10px] font-bold uppercase tracking-wider mb-1">Gains générés</p>
                <p className="text-white font-black text-xl leading-none">{earnings.toLocaleString()}</p>
                <p className="text-blue-300 text-[10px] font-semibold mt-1">{currency} au total</p>
              </div>
            </div>
          </div>
        </motion.div>
