...
import {
  ...
  PartyPopper, ExternalLink, MessageSquare,
} from "lucide-react";
...
export default function Activate() {
  ...
  const [otpReference, setOtpReference] = useState(""); // AshtechPay reference for OTP retry
  ...
  // Load operators from server (proxies AshtechPay /v1/countries)
  useEffect(() => {
  ...
  // Poll payment status in waiting phase
  useEffect(() => {
    if (phase !== "waiting" || !transactionId) return;
    const interval = setInterval(() => checkPaymentStatus(), 6000);
  ...
  const handlePay = async () => {
    ...
    try {
      const resp = await fetch("/api/activate/initiate", {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ country, phone: phone.trim(), operator: selectedOperator }),
      });
  ...
  const handleSubmitOtp = async () => {
    ...
    try {
      const resp = await fetch("/api/activate/otp", {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ country, phone: phone.trim(), operator: selectedOperator, otp: otp.trim(), reference: otpReference }),
      });
      const json = await resp.json() as any;
  ...
  const canPay = country && phone.trim().length >= 6 && selectedOperator && !loadingOperators;
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-start bg-gradient-to-br from-[#0D1B3E] via-[#1565C0] to-[#0D1B3E] p-4 py-8">
      ...
      >
        <img src={`${BASE}logo.png`} alt="Nexarix" className="h-20 w-20 object-contain drop-shadow-2xl mb-3" />
        <p className="text-white text-xl font-black tracking-tight">NEXARIX</p>
        <p className="text-blue-200 text-sm mt-0.5 font-medium tracking-wide">Activation du compte</p>
      </motion.div>
      ...
      <div className="w-full max-w-sm space-y-4">
          ...
          <div className="p-6">
            ...
            {paymentMode === "auto" ? (
              <AnimatePresence mode="wait">
                ...
                {(phase === "form" || phase === "initiating") && (
                  <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
                    ...
                    <div className="border-t border-gray-100 pt-3">
                      ...
                      >
                        <SiWhatsapp className="h-4 w-4" />Besoin d'aide ? Support WhatsApp
                      </a>
                    </div>
                  </motion.div>
                )}
                  ...
                  <motion.div key="otp" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
                    {ussdCode ? (
                      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-center">
                        <p className="text-sm font-black text-amber-700 mb-1">📲 Code USSD à composer</p>
                        <p className="text-xs text-amber-600 mb-2">Composez ce code sur votre téléphone pour recevoir votre OTP :</p>
                        <div className="bg-white rounded-xl px-4 py-3 border border-amber-200 inline-block">
                          <p className="font-black text-xl tracking-widest text-amber-800">{ussdCode}</p>
                        </div>
                        <p className="text-xs text-amber-500 mt-2">Après avoir composé, saisissez le code reçu ci-dessous.</p>
                      </div>
                    ) : (
                ...
                {/* ── WAVE ── */}
                {phase === "wave" && (
                  <motion.div key="wave" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
                    <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5 text-center">
                      <p className="text-sm font-black text-blue-700 mb-2">💳 Paiement Wave</p>
                      <p className="text-xs text-gray-500 mb-4">
                        Cliquez sur le bouton ci-dessous pour ouvrir l'application Wave et confirmer le paiement de{" "}
                        <span className="font-bold">{activationFee.toLocaleString("fr-FR")} FCFA</span>.
                      </p>
                      <a
                        href={waveUrl} target="_blank" rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-black rounded-2xl px-6 py-3 text-sm shadow-lg hover:-translate-y-0.5 transition-all"
                      >
                        <ExternalLink className="h-4 w-4" />Ouvrir Wave
                      </a>
                    </div>

                    <button
                      onClick={handleManualCheck} disabled={checking}
                      className="w-full h-12 rounded-2xl bg-gradient-to-r from-[#1565C0] to-[#1E88E5] text-white font-black flex items-center justify-center gap-2 shadow-lg disabled:opacity-70"
                    >
                      {checking ? <><Loader className="h-4 w-4 animate-spin" />Vérification…</> : <><RefreshCw className="h-4 w-4" />Vérifier mon paiement</>}
                    </button>
                ...
                {phase === "success" && (
                  ...
                  >
                    ...
                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.4 }}>
                      ...
                      <p className="text-sm text-gray-500 mt-1">Bienvenue sur Nexarix. Vous allez être redirigé…</p>
                    </motion.div>
                    <motion.div className="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
                      <motion.div className="h-full bg-emerald-500 rounded-full" initial={{ width: "0%" }} animate={{ width: "100%" }} transition={{ duration: 3, ease: "linear" }} />
                    </motion.div>
                    <button
                    ...
                    >
                      <CheckCircle2 className="h-4 w-4" />Accéder au tableau de bord
                    </button>
                  </motion.div>
                )}
              ...
              <div className="space-y-4">
                ...
                <a
                  href={publicSettings?.telegramLink || "#"} target="_blank" rel="noopener noreferrer"
                  className="w-full rounded-2xl bg-gradient-to-r from-green-500 to-emerald-600 text-white font-black flex items-center justify-center gap-2.5 shadow-lg shadow-green-200 hover:-translate-y-0.5 active:translate-y-0 transition-all py-3"
                >
                  <SiWhatsapp className="h-5 w-5" />Contacter le support WhatsApp
                </a>
              </div>
            )}
        ...
        {/* Logout */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3, duration: 0.4 }} className="text-center">
