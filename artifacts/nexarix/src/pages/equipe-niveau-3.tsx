...
export default function EquipeNiveau3() {
  const { user } = useAuth();
  const { data: downline, isLoading } = useGetDownline();
  const currency = getCurrencyCode(user?.country);

  const members = downline?.level3 ?? [];
  const earnings = downline?.mlmEarningsL3 ?? 0;
    ...
    <AppLayout>
      <div className="space-y-4 pb-10">
        ...
        <motion.div
          ...
          style={{ background: "linear-gradient(135deg, #3b0764 0%, #6d28d9 55%, #8b5cf6 100%)" }}
        >
          ...
          <div className="relative z-10 p-5 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-violet-200 text-[10px] font-bold uppercase tracking-widest">Mon Équipe</p>
                  <p className="text-white font-black text-lg leading-tight">Niveau 3</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-white font-black text-3xl leading-none">{count}</p>
                <p className="text-violet-200 text-[11px] font-semibold mt-0.5">membre{count > 1 ? "s" : ""}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-2xl border border-white/10 p-3 text-center" style={{ background: "rgba(255,255,255,0.08)" }}>
                ...
                <p className="text-violet-200 text-[10px] font-bold uppercase tracking-wider mb-1">Commission</p>
                <p className="text-white font-black text-xl leading-none">{CONFIG.commission.toLocaleString()}</p>
                <p className="text-violet-300 text-[10px] font-semibold mt-1">{currency} / filleul actif</p>
              </div>
              <div className="rounded-2xl border border-white/10 p-3 text-center" style={{ background: "rgba(255,255,255,0.08)" }}>
                <Users className="h-3.5 w-3.5 mx-auto mb-1.5 text-yellow-300" />
                <p className="text-violet-200 text-[10px] font-bold uppercase tracking-wider mb-1">Gains générés</p>
                <p className="text-white font-black text-xl leading-none">{earnings.toLocaleString()}</p>
                <p className="text-violet-300 text-[10px] font-semibold mt-1">{currency} au total</p>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div custom={1} variants={fadeUp} initial="hidden" animate="visible">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 px-1">
            {activeCount} actif{activeCount > 1 ? "s" : ""} sur {count} — Niveau 3
          </p>
          <MemberList
            members={members.filter((m: any) => m.status === "active")}
            isLoading={isLoading}
            levelConfig={CONFIG}
            emptyMessage="Aucun filleul de niveau 3 pour l'instant"
          />
        </motion.div>
        ...
        {members.filter((m: any) => m.status !== "active").length > 0 && (
          <motion.div custom={2} variants={fadeUp} initial="hidden" animate="visible">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 px-1">
              {members.filter((m: any) => m.status !== "active").length} inactif{members.filter((m: any) => m.status !== "active").length > 1 ? "s" : ""} — Niveau 3
            </p>
            <MemberList
              members={members.filter((m: any) => m.status !== "active")}
              isLoading={isLoading}
