...
export const loginLimiter = rateLimit({
  ...
  handler: (_req: Request, res: Response) => {
    res.status(429).json({ error: "Trop de tentatives de connexion. Réessayez dans 24h." });
  },
...
export const registerLimiter = rateLimit({
  ...
  keyGenerator: getClientIp,
  handler: (_req: Request, res: Response) => {
    res.status(429).json({ error: "Trop d'inscriptions depuis cette adresse. Réessayez dans 24h." });
  },
  standardHeaders: true,
  legacyHeaders: false,
});
...
export const adminLoginLimiter = rateLimit({
  ...
  keyGenerator: getClientIp,
  handler: (_req: Request, res: Response) => {
    res.status(429).json({ error: "Trop de tentatives admin. Réessayez dans 24h." });
  },
  standardHeaders: true,
  legacyHeaders: false,
});
...
export const otpLimiter = rateLimit({
  ...
  keyGenerator: getClientIp,
  handler: (_req: Request, res: Response) => {
    res.status(429).json({ error: "Trop de tentatives OTP. Réessayez dans 24h." });
  },
  standardHeaders: true,
  legacyHeaders: false,
});

/** Confirmation de retrait: 5 essais / 24h per IP */
export const withdrawalConfirmLimiter = rateLimit({
  windowMs: DAY_MS,
  max: 5,
  keyGenerator: getClientIp,
  handler: (_req: Request, res: Response) => {
    res.status(429).json({ error: "Trop de tentatives. Réessayez dans 24h." });
  },
  standardHeaders: true,
  legacyHeaders: false,
});
